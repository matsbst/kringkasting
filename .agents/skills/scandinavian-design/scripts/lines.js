...
function collect({ yMin, yMax }) {
  ...
  const visible = (c) => c.visibility !== "hidden" && c.display !== "none" && c.opacity !== "0";
  ...
  for (const el of document.querySelectorAll("*")) {
    ...
    for (const pseudo of ["::before", "::after"]) {
      ...
      const bord = (parseFloat(pc.borderTopWidth) > 0 && paints(pc.borderTopColor)) ||
...
(async () => {
  ...
  const report = (label, rows) => {
    ...
    for (const h of rows) {
      console.log(
        `  y=${String(h.y).padStart(6)} x=${String(h.x).padStart(5)} span=${
          String(
            h.span,
          ).padStart(5)
        }  ${h.mechanism.padEnd(13)} ${h.el}\n` +
          `${" ".repeat(24)}${h.detail}`,
      );
    }
  };
