...
(async () => {
  ...
  const found = await page.evaluate(
    ({ minSpread, maxSpread, exclude }) => {
      ...
      const composite = (p, bg) =>
        p.alpha >= 1 ? p.rgb : p.rgb.map((c, i) => Math.round(c * p.alpha + bg[i] * (1 - p.alpha)));
      ...
      const mediaSel = "picture, video, canvas, img, figure" + (exclude ? `, ${exclude}` : "");
  ...
  for (const h of tinted) {
    ...
    const note = h.alpha < 1 ? ` (declared ${h.declared} at ${h.alpha} alpha)` : "";
    console.log(
